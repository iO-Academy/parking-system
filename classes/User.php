...
class User {
    ...
    public function getBookings() {
        ...
        $sql = "SELECT `carpark`.`name` AS `Carpark Name`, 
                        DATE_FORMAT(`bookings`.`FROM`,'%d-%m-%Y') AS `Date From`, 
                        TIME_FORMAT(`bookings`.`FROM`, '%H:%i') AS `Time From`, 
                        DATE_FORMAT(`bookings`.`TO`,'%d-%m-%Y') AS `Date To`, 
                        TIME_FORMAT(`bookings`.`TO`, '%H:%i') AS `Time To` 
                FROM `bookings` LEFT JOIN `carpark` ON `bookings`.`carpark_id`=`carpark`.`id` 
